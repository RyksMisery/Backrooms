...
Свет является только визуальной атмосферой и постановкой. Яркость, мигание,
погасшая панель, цвет, тень и направление луча не кодируют правильный маршрут,
состояние загадки или обязательную подсказку. Пространственная задача обязана
оставаться решаемой без трактовки света как сигнала.
...
Единственное продуктовое исключение — область типа `pit`, служащая якорем
бесконечного провала. До reveal она живёт по обычному area-group. С момента
активации кольца сохранённые семьи якоря переходят во владение кольца и больше
не зависят от статической HUD/occupancy-области: иначе при входе координат в
`ЗАЛ-ПРОВАЛ` весь набор одновременно возвращался бы в topology-набор уже с
ненулевым весом. Каждая семья получает вес только от собственной позиции
светильника по той же симметричной дистанционной кривой, что секции кольца;
после ухода якоря за тёмную границу семьи удаляются. Панель, area-source,
bounce и fallback одной семьи используют общий fixture-anchor; направление и
история движения в вес не входят.
...
Размер области не участвует в этом правиле. Для произвольного footprint
сначала строится `ceiling_mask` из того же GridFrame, что потолочная текстура.
Светильник разрешён только если каждая клетка его footprint целиком входит в
эту маску и над ней существует соответствующая потолочная плитка. Выход панели
за контур, перекрытие выреза или попытка компенсировать несовпадение UV-сдвигом
считаются ошибкой валидации. После выгрузки и повторной сборки тот же
`SpaceSpec/GridFrame` обязан восстановить те же anchors панелей.
...
Продуктовый `office_corridor`, включая резидентную цель портала в колонне,
также подключает `level_e_area` и `LF3-11F`. Его офисная раскладка панелей не
является разрешением на локальный shadow-pool, source-profile или численные
переопределения света.
...
Итог ручного A/B `6/7/8 м` от `2026-08-12`: `level_e` сохраняет канонический
`AREA_LIGHT_BOUNCE_RANGE=8 м`. Значение `7 м` немного выразительнее в большом
зале, но недостаточно освещает области с редкими источниками, особенно зал-
провал. Локальный override по типу комнаты не вводится, чтобы не дробить общий
световой профиль. Крутилки `[`/`]` остаются только диагностическими.

## Передача света через визуальный портал

### Постоянно видимый фантом

Если портал остаётся видимым элементом композиции, его свет не является
кандидатом обычного дистанционного стриминга. Пока активен render-target:

- целевой канонический light-pool вычисляется от отдельного виртуального
  наблюдателя, закреплённого у `destination-anchor`, а не от физического
  расстояния игрока до удалённой области;
- portal-camera всё равно получает полный transform основной камеры —
  наблюдатель света не участвует в параллаксе;
- копии выбранных источников наследуют текущее состояние пула, параметры и
  тени, но всегда имеют `distance_fade_enabled=false` внутри proxy-world;
- запрещено принудительно включать все лампы цели: это устраняет гашение ценой
  пересвета и меняет авторский рисунок света;
- после физического перехода область возвращается к обычному наблюдателю
  игрока и своему штатному дистанционному пулу.

Следовательно, расстояние и экранный размер апертуры могут влиять на стоимость
представления только там, где `ViewSpec` разрешает его отключение. Для
закреплённого фантома они не имеют права менять яркость, состав светового пула
или активность render-target. Проверка выполняется
`validate_space_render_proxy_module.gd`, `validate_directed_gateway_pair.gd` и
дальним 90-кадровым маршрутом `run_directed_gateway_visual_bot.gd`.

`SubViewportTexture` переносит изображение области, но не её физический свет в
основной `World3D`. Если в момент пространственного свопа реальная область
скрывается, а ближайшая неизменяемая геометрия порога остаётся, её освещение
передаётся заранее прогретым light-only дублям исходных канонических семей.
Дубли сохраняют позиции, цвет, энергию, спад, outage/flicker и пространственный
fade оригинала; отдельный произвольно настроенный fill запрещён.

Для портала с полной подменой области и скрытием настоящей стороны используется выделенный
`PORTAL_LIGHT_RECEIVER_LAYER`: его получают только рама, толщина проёма и
непосредственное продолжение пола. Световые дубли используют
`light_cull_mask`, содержащий только этот слой. Портальная поверхность,
оболочка удалённой области и её собственные световые панели receiver-layer не
получают. Слой назначается только самостоятельным точным фрагментам порога;
назначение производному слитому mesh по пересечению AABB запрещено. Это правило
не распространяется на постоянно видимую апертуру, для которой ниже задан
отдельный режим. Визуальный портал проявляется пространственно внутри толщины стены:
его вес зависит только от signed-distance до порога и проходит `0 → 1` на
полосе `0.75 м`. Настоящая область остаётся активной до полного веса портала;
только затем в одном кадре передаёт свет дублям и скрывается. При возврате она
сначала восстанавливается под ещё непрозрачным порталом и передача света
выполняется обратно, после чего портал пространственно растворяется. Поэтому
не возникает ни двойной мощности, ни кадра без света, а неподвижная камера не
меняет состояние.

Камера visual-proxy копирует итоговый transform основной камеры только в
`RenderingServer.frame_pre_draw`, после camera leveling/bob персонажа. Иначе
proxy отстаёт на один кадр, что особенно заметно как смещение нижнего стыка при
сильном наклоне к полу. Пока вес пространственного blend находится между 0 и
1, render-target обновляется каждый кадр даже в мобильном профиле; его штатный
лимит частоты возвращается только для полностью проявленного портала.

Продуктовая щель `level_e` использует handoff чёрного куба: принятый
`infinite_corridor_e` рендерится в изолированном `World3D`, а ближайшие
активные источники его LF3-пула получают точные light-only копии в основном
мире. Копии сохраняют transform, цвет, энергию, радиус и спад источника;
handoff-тени отключены по тому же правилу, что у порога провала. Произвольный
fill и отдельные настройки света запрещены.
Visual-anchor щели лежит на внешней грани стены, сохраняя перед изображением
трёхклеточную толщину настоящего прохода. Ближайшая реальная семья
`infinite_corridor_e` занимает своё логическое место за этой плоскостью.
После нормализации proxy-world transform источника уже выражен относительно
visual-anchor и переносится в основной мир ровно один раз. Повторное применение
target-transform или дополнительный сдвиг на толщину стены запрещены: они
разводят видимую панель и её handoff-свет и создают границу на портале.
Для постоянно видимой щели `level_e` выделенный receiver-layer запрещён:
производный пол или стена могут быть слиты в крупный mesh, и назначение слоя по
AABB разрешает источнику осветить скрытую часть того же mesh за перегородкой.
Исходящая копия использует обычные world/ceiling layers и отбрасывает тень от
настоящей геометрии. Поэтому свет проходит только через физическую апертуру и
не может появиться с обратной стороны стены. Отдельный видимый receiver-liner
также не допускается.
В направлении `proxy → настоящий порог` переносится только исходный direct
ближайшей панели. Добавлять к нему отдельный bounce запрещено: одновременный
перенос direct+bounce дважды вносит одну семью и создаёт светлую ступень на
стыке, а bounce без direct меняет весь профиль настоящего тоннеля. Direct
синхронизирует фактическое runtime-состояние исходника, а его распространение
ограничивает настоящая геометрия порога.

Панели render-only коридора не наследуют локальную фазу циклических чанков.
Proxy формирует одну непрерывную последовательность от portal-anchor: центры
идут на диагонально чередующихся колонках с двумя пустыми потолочными клетками
между соседними рядами. Первый ряд является обычным членом этой
последовательности; отдельный сдвиг ближайшей панели запрещён.
Render-only представление щели не включает локальный LF3 floor-indirect:
этот эмиссионный слой существует только внутри proxy-world и не может быть
физически продолжен через порог, поэтому создаёт фотометрическую границу.
Прокси и настоящий проход используют общие direct+bounce компоненты;
полноценный посещаемый `infinite_corridor_e` сохраняет LF3 indirect без
изменений.

Явная visual anomaly продуктовой щели: внутри render-only коридора продольный
множитель direct-пула зафиксирован на `1.0` до конечного капа. Поэтому все
видимые ряды сохраняют полную штатную энергию и эмиссию панели независимо от
расстояния до proxy-player. Range и локальный attenuation всех источников
остаются каноническими; отдельный terminal fill и эмиссионная подсветка торца
запрещены. Handoff у порога остаётся прежним.
Посещаемый `infinite_corridor_e` это исключение не наследует и продолжает
гасить свет по своей дистанционной кривой.

Световой handoff визуального портала всегда двусторонний. Все реальные
источники принимающего пространства, чей радиус пересекает апертуру, получают
light-only копии в proxy-world с transform относительно того же portal-anchor.
Копии синхронизируют тип, цвет, энергию, радиус, спад, видимость и flicker, но
не отбрасывают вторую тень. Поэтому вход proxy получает фактический свет зала,
а не подобранную вручную яркость. Фиксированные seam-gain, tint или exposure,
маскирующие расхождение двух миров, запрещены. Геометрический overlap остаётся
только защитой от depth-щели и не участвует в светокоррекции.

Локальное исключение лаборатории `ceiling_cube_test`: апертура и proxy-коридор
имеют одинаковую ширину. Handoff двунаправленный и всё равно не вводит
произвольный fill. В основной `World3D` дублируется ближайший к апертуре ряд
канонических `level_e_area` bounce-источников коридора. В изолированный
proxy-world дублируется ближайший к стене ряд уже существующих средневысотных
`wide` fill-источников куба: именно он имитирует внешний свет, попадающий во
вход коридора. Дубли сохраняют исходные цвет, энергию, радиус и спад, но не
отбрасывают тени.
