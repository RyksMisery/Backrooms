...
- Уровни и области **не задают ambient своими числами**. `level_e` берёт
  значение через `ARCHITECTURE.AMBIENT_ENERGY`; рантайм-регулятор `-`/`+`
  остаётся отладочным и продуктовый профиль не переопределяет.
- Потребители, которым нужен более тёмный ambient, задают его **долей от
  канона**, а не абсолютным числом, и следуют за изменением канона
  автоматически (так сделан `echo_loop_lab`: `0.5 ×` канона).
