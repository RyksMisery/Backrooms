...
### A/B-перенос light-zone профиля в level_e

Первый продуктовый потребитель универсального профиля — только
переключаемый эксперимент `level_e`, а не замена принятого `LF3-11F`.
Клавиша `V` всегда доступна в текущей A/B-ветке и сравнивает
`LF3-11F ↔ ZONE-11` без перезапуска. Обычный запуск и запуск из редактора
стартуют в `LF3-11F`; при первом нажатии `V` data-only планы лениво строятся
после уже готовых occupancy и списка ceiling-bounce Omni. Аргумент
`--level-e-light-zones-ab` нужен только чтобы сразу стартовать в кандидате.
Состояние A/B не сохраняется.
...
профиле, но не занимают один из 11 shadow-slot. Для anchor-плана `level_e`
передаёт центроид собственного `area_group`: постоянные 11 выбираются около
интерьера, где находится игрок, а не размазываются по порталам всех соседей.
Лаборатория без anchor-focus сохраняет исходный portal-aware ranking.
