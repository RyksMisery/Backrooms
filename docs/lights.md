...
Запуск с `--level-e-light-zones-ab` строит data-only план один раз после
готовности occupancy и списка ceiling-bounce Omni, включает кандидат и
разрешает клавишей `V` сравнивать `LF3-11F ↔ ZONE-11`.
Обычный запуск без аргумента остаётся в `LF3-11F`; состояние A/B не
сохраняется.
