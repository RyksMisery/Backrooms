...
## Канонический ambient

`architecture_module.AMBIENT_ENERGY` — единственный источник значения ambient.
С `2026-07-30` он равен `0.005`: прежние `0.010` давали для `level_e` слишком
светлую заливку, темнота между источниками читалась недостаточно. Цвет
(`AMBIENT_COLOR`) не менялся.

Правила:

- **`0.005` — для всех и всегда.** Уровни, области и лаборатории не задают
  ambient ни своим числом, ни долей от канона: значение берётся только через
  `AMBIENT_ENERGY`. Отклонения нет ни у одного потребителя.
- Исключение возможно только как явный **кастомный** профиль по отдельному
  решению автора и с предварительной фиксацией здесь. До этого любое
  локальное ослабление/усиление ambient считается ошибкой.
- Рантайм-регулятор `-`/`+` остаётся отладочным и продуктовый профиль
  не переопределяет.
- `AMBIENT_ENERGY_OLD = 0.08` — легаси-эталон старого света, не трогать.

История: до `2026-07-30` канон был `0.010`, а `echo_loop_lab` и
`infinite_corridor_e` держали собственный, вдвое более тёмный ambient. Обе
локальные настройки сняты вместе с уменьшением канона.
...
## Обязательный световой default новой области

Новая область не выбирает собственные панель, энергию, range, attenuation,
shadow-профиль или семейство источника. При
`construction_profile=canonical` полный AreaSpec-потребитель и
`standard_area_module.gd` автоматически подключают канонические панели,
семейство `level_e_area` и runtime `LF3-11F`. Локальная спецификация описывает
только тип пространства и, при необходимости, документированный рисунок
размещения; численные параметры источников остаются у `lighting_module.gd`.

Ранее принятый свет сохраняется только у области, явно помеченной
...
- `hall`: одиночные панели регулярной сеткой; препятствия и симметричное
  удаление пары имеют приоритет над заполнением;
- `corridor`: одиночные либо явно объявленные двойные панели вдоль центральной
  оси чистой трассы; поперечный световой мост разрешается только при
  документированной достаточной ширине;
- `office`/`utility`: ближайшая допустимая к геометрическому центру клетка
  либо симметричная пара относительно продольной оси;
- `junction`: отдельный симметричный рисунок, согласованный со всеми рукавами;
- `maze`: существующая прямая occupancy-проверка по логическим клеткам.

Во всех рисунках действует один порядок приоритетов: каноническая сетка →
occupancy и зазор → тип пространства и направление маршрута → ритм →
симметрия. Область не исправляет конфликт произвольным сдвигом панели.
...
Между фактическим footprint любой панели и ближайшей стеной, перегородкой или
колонной всегда остаётся минимум одна полностью свободная клетка occupancy.
Это правило не получает локальных исключений из-за ширины прохода, желаемой
яркости или ритма света. Нарушение допускается только после отдельного прямого
указания пользователя для конкретного случая.
...
Для `construction_profile=canonical` это означает выбор или параметризацию
одного из общих рисунков выше. Полностью ручной рисунок является явным
локальным override с причиной; он не становится default следующей области.
...
## Гашение света по дальности: свечением, а не `visibility_range`

Правило (зафиксировано). Там, где источник и его потолочная панель гаснут по
расстоянию до игрока (бесконечные пространства, стриминг, тредмилл), панель
обязана гаснуть **изменением собственного свечения**, а не диапазоном
видимости меша.

Почему. `GeometryInstance3D.visibility_range_fade_mode` в Godot реализован
**дизерингом** — панель растворяется шумом по пикселям. Вблизи панель крупная,
и шум читается как плавное исчезновение; вдали панель занимает несколько
пикселей, и тот же шум выглядит как **мерцание дальних ламп**, после которого
«резко включается» световой пул области. Дефект не в свете: суммарная энергия
ламп при этом меняется плавно.

Как правильно:

- Панель и её источники ведёт **одна кривая** `level` от расстояния —
  один и тот же smoothstep, одна и та же точка отсчёта (сама панель) и одна
  и та же 3D-дистанция. Разные метрики для меша и лампы дают светящееся
  пятно там, где панель уже исчезла.
- Источник гасится множителем энергии, панель — `albedo_color` и
  `emission_energy_multiplier`.
- Материал панелей общий, поэтому личная копия выдаётся только на время
  перехода: полностью зажжённой панели возвращается общий материал, полностью
  погашенная просто скрывается.
- Пороги мерцания (`FLICK_PANEL_MIN_LEVEL`,
  `FLICK_PANEL_EMISSION_MIN_LEVEL`) применяются только к мерцанию — панель
  тускнеет, но не гаснет в ноль. К дальности они не применяются: дальняя
  панель обязана погаснуть полностью.
- Лампа гаснет чуть раньше своей панели (`LIGHT_LEAD_DISTANCE`), иначе на
  границе остаётся висеть световое пятно без видимого источника.

Reference implementation: `modules/infinite_pit_module.gd` —
`_update_light_fade`, `_apply_panel_level`.
