...
## Фактическая цепочка до первого разреза
...
После полного отделения и итогового машинного паритета `project.godot`
запускает `level_e.tscn`.
...
Статус: **выполнено 2026-07-27**. `level_e.gd` напрямую наследует
`level_areas_c.gd`; все 39 констант и 22 функции слоя `level_d` присутствуют
в продуктовой реализации. `level_d` не изменён.
...
Статус: **выполнено 2026-07-27** механическим плоским разворачиванием.
Перекрытые base-реализации сохранены как локальные `_level_e_base_*` hooks,
поэтому порядок lifecycle и виртуальная диспетчеризация не изменились.
`level_e.gd` не загружает `level_d.gd` или `level_areas_c.gd`.
...
Статус: **выполнено** после машинного паритета. Старые scenes/scripts не
удалены и остаются референсами.
...
- Forward+ streaming A/B `16-42-08`: collision counts и mesh-signature
  совпали для всех 12 пар; counts совпадают с baseline до разреза;
- `LF3_LEVEL_E_SHADOW_PROFILE_OK`, продуктовый профиль восстановлен в
  `LF3-11F`, пик handoff — 11;
- integration `16-42-36`: `passed=true`, 9 recycle-циклов, все состояния
  восстановлены, main commit mean/max `0.427/0.557 ms`.

После полного разворачивания `level_areas_c`:

- `level_e.gd`: `extends Node3D`, старых preload/load нет;
- Forward+ streaming A/B `16-48-17`: все 12 signatures и collision counts
  совпали не только внутри sync/candidate, но и побайтно с `16-42-08`;
